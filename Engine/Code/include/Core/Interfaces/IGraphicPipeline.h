	...
	namespace Core
	{
		...
		namespace RHI
		{
			...
			public:
				...
				ENGINE_API virtual void Create(ILogicalDevice* a_logicalDevice, IRenderPass* a_renderPass, PipelineType a_type, std::array<IShader*, 2> a_vertFragShaders, std::vector<IShader*> a_additionalShaders = {}, bool a_expectVertexBuffers = true) = 0;
				...
				ENGINE_API virtual void  BindObjects(RHI::ICommandPool* a_commandPool, uint32_t a_commandBufferIndex, uint32_t a_currentFrame, uint32_t  a_imageIndex, std::vector<RHI::IBuffer*> a_indexBuffers,
													std::vector<RHI::IBuffer*> a_vertexBuffers, std::vector<uint32_t> a_indicesCount, std::vector<RHI::IObjectDescriptor*> a_objectsDescriptors, bool a_expectVertexBuffer = true) = 0;
