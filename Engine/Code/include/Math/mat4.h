...
namespace TheCoolerMath
{
	...
	public:
		...
		quat Diagonal() const; // method that returns the diagonal
		float Trace() const; // method that returns the trace
		...
		float Determinant() const; // returns the determinant of a matrix
		mat4 Comatrix() const;
		...
		static mat4 TRS(const vec3& a_translation, const quat& a_rotation, const vec3& a_scaling); // returns a TRS matrix
	...
	{
		vec3 t_vR;
		t_vR.x = (a_m.mElements[0] * a_v.x) + (a_m.mElements[1] * a_v.y) + (a_m.mElements[2] * a_v.z) + a_m.mElements[3];
		t_vR.y = (a_m.mElements[4] * a_v.x) + (a_m.mElements[5] * a_v.y) + (a_m.mElements[6] * a_v.z) + a_m.mElements[7];
		t_vR.z = (a_m.mElements[8] * a_v.x) + (a_m.mElements[9] * a_v.y) + (a_m.mElements[10] * a_v.z) + a_m.mElements[11];
		return t_vR;
	}
	...
	{
		quat t_qR;
		t_qR.x = (a_m.mElements[0] * a_q.x) + (a_m.mElements[1] * a_q.y) + (a_m.mElements[2] * a_q.z) + (a_m.mElements[3] * a_q.w);
		t_qR.y = (a_m.mElements[4] * a_q.x) + (a_m.mElements[5] * a_q.y) + (a_m.mElements[6] * a_q.z) + (a_m.mElements[7] * a_q.w);
		t_qR.z = (a_m.mElements[8] * a_q.x) + (a_m.mElements[9] * a_q.y) + (a_m.mElements[10] * a_q.z) + (a_m.mElements[11] * a_q.w);
		t_qR.w = (a_m.mElements[12] * a_q.x) + (a_m.mElements[13] * a_q.y) + (a_m.mElements[14] * a_q.z) + (a_m.mElements[15] * a_q.w);
		return t_qR;
	}
	...
	inline void mat4::Print() const
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			quat line(mElements[i], mElements[i + 1], mElements[i + 2], mElements[i + 3]);
			line.BlankPrint();
		}
		std::cout << '\n';
	}
	...
	inline void mat4::Transpose()
	{
		for (int i = 0; i < 4; ++i) {
			for (int j = i + 1; j < 4; ++j) {
				const float temp = mElements[i * 4 + j];
				mElements[i * 4 + j] = mElements[j * 4 + i];
				mElements[j * 4 + i] = temp;
			}
		}
	}
	...
	{
		float t_det = 0.f;
		t_det += mElements[0] * Mat3Determinant({ mElements[5], mElements[6], mElements[7], mElements[9], mElements[10], mElements[11], mElements[13], mElements[14], mElements[15] });
		t_det -= mElements[1] * Mat3Determinant({ mElements[4], mElements[6], mElements[7], mElements[8], mElements[10], mElements[11], mElements[12], mElements[14], mElements[15] });
		t_det += mElements[2] * Mat3Determinant({ mElements[4], mElements[5], mElements[7], mElements[8], mElements[9], mElements[11], mElements[12], mElements[13], mElements[15] });
		t_det -= mElements[3] * Mat3Determinant({ mElements[4], mElements[5], mElements[6], mElements[8], mElements[9], mElements[10], mElements[12], mElements[13], mElements[14] });
		return t_det;
	}
	...
	inline mat4 mat4::Orthographic(const float a_top, const float a_bottom, const float a_right, const float a_left, const float a_far, const float a_near)
	{
		return mat4({
