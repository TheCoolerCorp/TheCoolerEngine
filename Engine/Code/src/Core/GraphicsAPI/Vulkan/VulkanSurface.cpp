	...
	namespace Core
	{
		namespace GraphicsAPI
		{
			...
			void VulkanSurface::SetupInfo(RHI::IPhysicalDevice* a_physicalDevice)
			{
				...
				VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(t_device, m_surface, &m_capabilities), "Failed to get Surface capabilities");
				...
				if (t_formatCount != 0) {
					m_formats.resize(t_formatCount);
					VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(t_device, m_surface, &t_formatCount, m_formats.data()), "Failed to get Surface capabilities");
				}

				if (t_presentModeCount != 0) {
					m_presentModes.resize(t_presentModeCount);
					VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(t_device, m_surface, &t_presentModeCount, m_presentModes.data()), "Failed to get Surface capabilities");
				}
			}
