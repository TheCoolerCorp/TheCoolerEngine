...
namespace Engine
{
	namespace GamePlay
	{
		Camera::Camera(Math::vec3 a_up, Math::vec3 a_center, Math::vec3 a_eye, float a_fovY, float a_aspect,
			float a_near, float a_far, float a_speed, float a_sensitivity, bool a_freeCam) : m_up(a_up), m_center(a_center), m_eye(a_eye), m_fovY(a_fovY),
			m_aspect(a_aspect), m_near(a_near), m_far(a_far), m_speed(a_speed), m_sensitivity(a_sensitivity), m_isFreeCam(a_freeCam)
		{
